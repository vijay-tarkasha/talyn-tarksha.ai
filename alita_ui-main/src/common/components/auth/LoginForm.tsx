...
const LoginForm = () => {
  ...
  const handleClick = () => {
    ...
    if (req.username != '' && req.password != '') {
      axios.post(endPoint, req).then((d: any) => {
        if (d) {
      ...
      <form onSubmit={handleSubmit} noValidate>
        <PalmyraNewForm endPoint={endPoint} ref={formRef}>
          ...
          <div className="mt-3 text-sm text-left">
            <span className='login-forgot-pwd-text text-base'>{lfTexts?.footer?.signUpText}</span>
            <span
          ...
          <div className="mt-2 text-sm text-left">
            <span className='login-forgot-pwd-text text-base'>{lfTexts?.footer?.resetPwdText}</span>
            <span
              className="cursor-pointer ml-1 login-forgot-pwd-link font-semibold"
