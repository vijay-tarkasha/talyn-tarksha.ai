...
const clientRenderer = (data: any) => {
    ...
    return <span>
        <div className='grid'>
            <ProfileImage name={d.company_name} id={d.id} className={'profile_sm'}/>
        </div>
    </span>;
}
...
const UserGrid = (_props: IUserGridInput) => {
    ...
    const getPluginOptions = (): any => {
        return { customAddText: btnTexts?.user?.create }
    }
    ...
    return (
        <div className="page-right-left-container">
            <PalmyraGrid DataGridControls={TalynSummaryGridControls}
                columns={fields} title={usersTexts.grid.title} pageSize={[10, 20, 30]}
                ref={gridRef} getPluginOptions={getPluginOptions}
                endPoint={apiEndPoint} DataGridControlProps={{ newRecord }} />
        </div>
    )
}
